...
app = Flask(__name__) # Creamos una instancia de la clase Flask, que representa la aplicación web

@app.route('/') # Definimos una ruta para la raíz de la aplicación, usando el decorador app.route
def hola_mundo(): # Definimos una función que se ejecutará cuando se acceda a la ruta
    return 'Hola, mundo' # Devolvemos el mensaje "Hola, mundo"
...
def frotar_bayeta(n_frases): # Definimos una función que se ejecutará cuando se acceda a la ruta, recibiendo el parámetro n_frases
    ...
    return jsonify(frases) # Devolvemos la lista de frases convertida en JSON
